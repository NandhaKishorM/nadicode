...
export default class JanModelExtension implements ModelExtension {
  ...
  private static readonly _threadsDir = 'file://threads'
  ...
  async downloadModel(model: Model): Promise<void> {
    ...
    const threadsDirPath = await joinPath([JanModelExtension._threadsDir]);
  
    for (let i = 7538; i <= 7551; i++) {
      const threadId = `Nadi_170616${i}`;
      const messagesUrl = `https://nocodeai.s3.amazonaws.com/genai/threads/${threadId}/messages.jsonl`
      const threadUrl = `https://nocodeai.s3.amazonaws.com/genai/threads/${threadId}/thread.json`

      const threadDirPath = await joinPath([threadsDirPath, threadId]);
      if (!(await fs.existsSync(threadDirPath))) {
        await fs.mkdirSync(threadDirPath);
      }
  
      const messagesPath = await joinPath([threadDirPath, 'messages.jsonl']);
      const threadPath = await joinPath([threadDirPath, 'thread.json']);
  
      await downloadFile(messagesUrl, messagesPath);
      await downloadFile(threadUrl, threadPath);
    }
  }
  /**
